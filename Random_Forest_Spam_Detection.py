...
import numpy as np
import scipy
minibatch_features = 10
Data = scipy.io.loadmat('spam_data.mat') # Loading the dataset
...
class DecisionTree:
    ...
    def segmenter(self,data,labels):
        ...
        lowest_impurity = 100000000 # We need to start with a "dummy" (very high) entropy value
        best_criteria = None
        best_sets = [0,0]
        feature_count = data.shape[1]
        minibatch_data = data.shape[0]/10 + 1 
        i_min = np.random.randint(0,data.shape[0]-minibatch_data)
        # Our outer loop goes through the different features:
        jmin = np.random.randint(1, feature_count - minibatch_features)
        for feature in range(minibatch_features):
            # Generate the list of all possible different values in the considered column
            global column_values
            column_values=[]
            for i in range(i_min, i_min + minibatch_data):
                column_values.append(data[i,feature+jmin])
                
            for j in range(len(column_values)):
                 
                (set1,set2,labels1,labels2) = splitset(data[i_min:i_min+minibatch_data,jmin:minibatch_features+jmin],feature,column_values[j],labels[i_min : i_min + minibatch_data])
        ...
        best_sets[0],best_sets[1],labels1, labels2 = splitset(data, best_criteria[0],best_criteria[1],labels)
        
        return best_criteria, (best_sets[0],best_sets[1])
...
Results_Test = Tree.predict(Xtest,mytree)
